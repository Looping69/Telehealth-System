    ...
    <Card withBorder padding="md" radius="md" style={{ height: '100%' }}>
      <Stack gap="md">
        <Group justify="space-between" align="flex-start">
          <Group gap="sm">
            ...
            <Stack gap={2}>
              ...
              <Text size="xs" c="dimmed">{provider.type}</Text>
            </Stack>
          </Group>
          <Group gap="xs">
            <ActionIcon variant="light" size="sm" onClick={() => onView(provider)}>
              <Eye size={14} />
            </ActionIcon>
            <ActionIcon variant="light" size="sm" onClick={() => onEdit(provider)}>
              <Edit size={14} />
            </ActionIcon>
          </Group>
        </Group>
        ...
        <Stack gap="xs">
          <Group gap="xs">
            <Phone size={14} />
            <Text size="xs">{provider.phone}</Text>
          </Group>
          <Group gap="xs">
            <Mail size={14} />
            <Text size="xs">{provider.email}</Text>
          </Group>
          <Group gap="xs">
            <MapPin size={14} />
            <Text size="xs">{provider.address}</Text>
          </Group>
        </Stack>
...
const NewCoverageForm: React.FC<NewCoverageFormProps> = ({ onSubmit, onCancel, loading = false }) => {
  const form = useForm({
    initialValues: {
